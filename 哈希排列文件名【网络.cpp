// 哈希表，也称散列表。通过散列函数将数据值和它的存储位置联系起来。即通过精心地向表载入元素实现，
// 从而提高访问速度。本例中采取的解决冲突的办法是建立一个链表，在这个位置的后面，所有散列函数值为这个位置的元素都添加到这个链表中，
//码实现了将文件名按字母a-z分类，不区分大小写。挂可以从头部插入也可以从尾部追加，甚至可以再这个位置后面再挂一个散列表。
//代先以数组存储各节点，当发生冲突后即将节点加入链表中。然后遍历显示所有的文件名。
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#define HASHSIZE 26
